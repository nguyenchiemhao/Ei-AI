...
- 2026-09-15 — **rule 3 has nowhere to send a module.** It reads *"modules do not import each other's services; only through `ports/`"*, but [detail §7](../ei-ai-phase-1-detail.md) says `ports/` holds *"model-provider · vector-store · storage — only three seams"* and [design §5.4](../../design/ei-ai-agentic-knowledge-assistant.md) forbids adding a fourth: *"Không gì khác được trừu tượng hoá."* Those three are outbound infrastructure seams. A module that legitimately needs another module — `retrieval` needing workspace membership, `ingestion` needing `workspaces` — has no port to go through and no permitted way to build one.
